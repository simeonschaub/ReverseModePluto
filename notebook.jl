...
function show_steps(steps; height=400)
	...
	<span style="color: red; font-size: 1.5em; margin-bottom: 1em; margin-left: 10px">Move me!</span><br>

	<span style="width: 100%; display: flex; padding: 10px">
	<input $((id="f", type="button", value="<<"))/>
	<input $((id="m", type="button", value="<"))/>
	<input $((id="s", type="range", min=0, max=length(steps)-1, value=0)) style="vertical-align: middle; flex-grow: 1"/>
	<input $((id="p", type="button", value=">"))/>
	<input $((id="l", type="button", value=">>"))/>
	<span id="t" style="margin-left: 1em; width: $(ndigits(length(steps)-1))em; text-align: end">1</span>
	<script>
	const steps = $(to_json.(steps));
	...
	function set(val){
		val = Math.min(Math.max(val, 0), $(length(steps)-1));
		simple_chart_config.nodeStructure = steps[val];
		document.getElementById("$id1").innerHTML = "";
		my_chart = new Treant(simple_chart_config, null, \$);
		slider.value = val
		span.querySelector("#t").textContent = val + 1
		span.value = val
		span.dispatchEvent(new CustomEvent("input"))
	}

	span.querySelector("#f").onclick = () => set(0)
	span.querySelector("#l").onclick = () => set($(length(steps)-1))
...
/* Force Treant SVG connection lines to be visible */
.Treant path {
	stroke: #4A5568 !important; /* Line color (e.g., dark gray) */
	stroke-width: 2px !important;
	fill: none !important;
}

/* Ensure the container retains relative positioning for correct line alignment */
.chart {
...
manifest_format = "2.0"
project_hash = "de6078bb97f7e79a11ceeafdc20e6ef4c820771b"
